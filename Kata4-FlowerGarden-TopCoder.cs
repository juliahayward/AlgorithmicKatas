...
{
    // https://community.topcoder.com/stat?c=problem_statement&pm=1918
    [TestFixture]
    public class FlowerGardenTests
    {
        ...
        {
            var actualOrdering = GetOrdering(height, bloom, wilt);

            CollectionAssert.AreEqual(expectedOrdering, actualOrdering);
        }


        private int[] GetOrdering(int[] height, int[] bloom, int[] wilt)
        {
            ...
            {
                Flowers[i] = new Flower(i, height[i], bloom[i], wilt[i]);
            }

            // Blocked holds the indexes of the flowers this one must not go in front of;
            // BlockedBy holds the reverse relationship
            for (int i = 0; i < height.Length; i++)
            { 
                for (int j = 0; j < height.Length; j++)
                {
                    if (height[j] > height[i] && bloom[j] <= wilt[i] && wilt[j] >= bloom[i])
                    {
                        Flowers[i].BlockedBy.Add(Flowers[j]);
                        Flowers[j].Blocks.Add(Flowers[i]);
                    }                  
                }
            }

            // Sort the flowers into the order which we'd like to plant them...
            var SortedFlowers = Flowers.OrderByDescending(x => x.Height).ToList();

            // We plant the tallest that we can, without blocking any remaining flowers.
            while (SortedFlowers.Any())
            {
                var bestFlower = SortedFlowers.First(x => !x.Blocks.Any());
                PlantedFlowers.Add(bestFlower);
                SortedFlowers.Remove(bestFlower);
                // Once this flower is planted, ones that would block it no longer care.
                foreach (var blockingFlower in bestFlower.BlockedBy)
                    blockingFlower.Blocks.Remove(bestFlower);
            }

            return PlantedFlowers.Select(x => x.Height).ToArray();
        }
    }
    ...
    internal class Flower
    {
        ...
        internal int Bloom { get; set; }
        internal int Wilt { get; set; }
        internal List<Flower> BlockedBy { get; }
        internal List<Flower> Blocks { get; }

        internal Flower(int index, int height, int bloom, int wilt)
        {
            Index = index;
            Height = height;
            Bloom = bloom;
